...
import { MapPin, mapPinWrapper } from './MapPin';
...
class ReactiveOpenStreetMap extends Component {
  ...
  getMarkers = ({ showMarkers, renderData, defaultPin, onPopoverClick, resultsToRender, getPosition }) => {
    if (showMarkers) {
            ...
            openStreetMarkerProps.icon = new DivIcon({
              html: ReactDOMServer.renderToStaticMarkup(
                <div className={mapPinWrapper}>
                  ...
                  />
                  {onPopoverClick ? (
                    <OpenStreetPopup {...openStreetPopupPorops}>{onPopoverClick(item)}</OpenStreetPopup>
                  ) : null}
                </div>
              ),
            });
            return (
              <OpenStreetMaker key={item._id} position={[position.lat, position.lng]} {...openStreetMarkerProps}>
                {onPopoverClick ? (
                  <OpenStreetPopup {...openStreetPopupPorops}>{onPopoverClick(item)}</OpenStreetPopup>
                ) : null}
              </OpenStreetMaker>

...
class ReactiveMap extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!isEqual(this.props.center, prevProps.center)) {
      ...
      this.props.setMapData(this.props.componentId, this.getGeoQuery(this.props), persistMapQuery, forceExecute);
    }
    ...
    if (prevProps.defaultQuery && !isEqual(this.props.defaultQuery(), this.defaultQuery)) {
      const options = getQueryOptions(prevProps);
      options.from = this.state.from;
      this.defaultQuery = prevProps.defaultQuery();
    ...
    if (!this.props.pagination && prevProps.hits && this.props.hits && this.props.hits.length < prevProps.hits.length) {
      if (this.props.onPageChange) this.props.onPageChange();

      updatedState.from = 0;
    }

    if (this.props.pagination && prevProps.total !== this.props.total) {
    ...
    if (
      this.props.defaultZoom !== prevProps.defaultZoom && !isNaN(this.props.defaultZoom) && // eslint-disable-line
  ...
  getGeoQuery = (props = this.props) => {
    ...
    if (!this.props.mapRef) return this.defaultQuery ? this.defaultQuery.query : null;
  ...
  setGeoQuery = (executeUpdate = false) => {
    ...
    if (executeUpdate || (!this.skipBoundingBox && !this.state.mapBoxBounds)) {
      ...
      this.props.setMapData(this.props.componentId, this.defaultQuery, persistMapQuery, forceExecute);
    }
  ...
  setPage = page => {
    ...
    if (this.props.URLParams) {
      this.props.setPageURL(`${this.props.componentId}-page`, page + 1, `${this.props.componentId}-page`, false, true);
    }
  };
  ...
  getCenter = hits => {
    ...
    if (hits && hits.length) {
      ...
      return hits[0] && hits[0][this.props.dataField] ? this.getPosition(hits[0]) : this.getDefaultCenter();
    }
  ...
  handleOnDragEnd = () => {
    if (this.state.searchAsMove) {
      this.setState({ preserveCenter: true }, () => this.setGeoQuery(true));
    }
    if (this.props.mapProps.onDragEnd) this.props.mapProps.onDragEnd();
  };

  renderError = () => {
    const { error, renderError, isLoading } = this.props;
